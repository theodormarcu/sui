...
module Sui::ValidatorSet {
    ...
    struct ValidatorSet has store {
        /// Total amount of stake from all active validators (not include delegation),
    ...
    public(friend) fun advance_epoch(
        self: &mut ValidatorSet,
        computation_reward: &mut Coin<SUI>,
        ctx: &mut TxContext,
    ) {
        distribute_reward(&mut self.active_validators, self.validator_stake, computation_reward, ctx);
    ...
    fun distribute_reward(validators: &mut vector<Validator>, total_stake: u64, reward: &mut Coin<SUI>, ctx: &mut TxContext) {
        let total_reward_amount = Coin::value(reward);
        let length = Vector::length(validators);
        let i = 0;
        while (i < length) {
            let validator = Vector::borrow_mut(validators, i);
            ...
            let reward = Coin::withdraw(reward, reward_amount, ctx);
